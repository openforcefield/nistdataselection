...
from nistdataselection.utils import SubstanceType, log_filter
...
def main():
    ...
    for property_type, substance_type in target_substances_per_property:
        ...
        # Filter out any non-aqueous property measurements.
        with log_filter(data_set, "non-aqueous data points were filtered out."):
            filtering.filter_by_smiles(
                data_set, smiles_to_include=["O"], smiles_to_exclude=None
            )
    ...
    target_property_state_points = {
        (EnthalpyOfMixing, SubstanceType.Binary): target_state_points,
        (ExcessMolarVolume, SubstanceType.Binary): target_state_points,
    }

    # Set the output path to the data set.
    data_set_name = "aqueous_data_set"
