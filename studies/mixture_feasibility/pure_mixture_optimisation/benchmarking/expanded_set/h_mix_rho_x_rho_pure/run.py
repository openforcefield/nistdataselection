...
def generate_request_options(data_set, force_field_source):
    """Decreases the target mass density of the protocol which
    builds the gas phase coordinates as part of the enthalpy of
    vaporization workflow. Note this will only affect the coordinate
    building, as the gas phase simulations are performed with PBC
    disabled.

    This is a workaround for a current bug in the evaluator framework.
    """

    request_options = EvaluatorClient.default_request_options(
        data_set, force_field_source
    )

    h_vap_schema = request_options.calculation_schemas["EnthalpyOfVaporization"][
        "SimulationLayer"
    ]
    build_gas_coords_schema = next(
        iter(
            [
                x
                for x in h_vap_schema.workflow_schema.protocol_schemas
                if x.id == "build_coordinates_gas"
            ]
        )
    )
    build_gas_coords_schema.inputs[".mass_density"] = 0.01 * unit.gram / unit.milliliter
...
def main():
    ...
    with calculation_backend:
        ...
        with server:
            ...
            request_options = generate_request_options(data_set, force_field_source)
